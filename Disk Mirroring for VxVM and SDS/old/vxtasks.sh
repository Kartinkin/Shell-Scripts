...
#		если не установлен dedicated dump device
...
#		Если dump device уже установлен на вторую половину зеркала,
...
#CurDumpDsk=${CurDumpDev##*/}
#CurDumpDsk=${CurDumpDsk%s*}

#if [[ ${CurDumpDsk
#	eval dumpadm -u -d ${Dsk}
#	/usr/lib/vxvm/bin/vxswapctl set ${Dsk}
#	cp /etc/dumpadm.conf /etc/vx/reconfig.d/saveconf.d/etc/dumpadm.conf.new
}
...
#	print "\tSwap slices are ${SwapDevs}."
	...
	if [[ -n ${EEE} || ${Force} == 1 || -z ${CurDump} || ${CurDump} == swap ]]
	then
